...
from luoyang.yolov7.YoloV7 import YoloV7
...
def torch_detect():
    param = Yolo7Param(m_type="l")
    model_path = "../../yolo_model/yolo_v7_l.pth"
    ...
    yolo_v7 = YoloV7(param=param)
    yolo_v7.load_state_dict(torch.load(model_path))
    yolo_v7.eval()
    yolo_v7.cuda()
    predicts = yolo_v7.predict(tensor_dict={"batch_images": torch.tensor([data_dict["image"]]).cuda()})["predicts"]
...
def to_onnx():
    ...
    yolo_v7.eval()
    yolo_v7.fuse()
    onnx_path = "../../yolo_model/yolo_v7_l.onnx"
...
def onnx_video():
    ...
    while True:
        ...
        cv2.imshow("yolo_v7_video", frame)
        cv2.waitKey(1)
