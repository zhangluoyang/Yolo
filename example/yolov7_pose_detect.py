...
import torch
from typing import *
from luoyang.yolov7_pose.YoloV7Pose import YoloV7Pose
...
def torch_detect():
    param = Yolo7PoseParam(m_type="l")
    param.conf_threshold = 0.5
    model_path = "../../yolo_model/yolo_v7_l_pose.pth"
    image_path = (r"../resource/face.jpg")
    ...
    yolo_v7_pose = YoloV7Pose(param=param)
    yolo_v7_pose.load_state_dict(torch.load(model_path))
    yolo_v7_pose.eval()
    yolo_v7_pose.cuda()
    # yolo_v7_pose.fuse()
    predicts = yolo_v7_pose.predict(tensor_dict={"batch_images": torch.tensor([data_dict["image"]]).cuda()})["predicts"]
...
def to_onnx():
    ...
    param = Yolo7PoseParam(m_type="l")
    model_path = "../../yolo_model/yolo_v7_l_pose.pth"
    yolo_v5_pose = YoloV7Pose(param=param)
    yolo_v5_pose.load_state_dict(torch.load(model_path, map_location="cpu"))
    yolo_v5_pose.eval()
    yolo_v5_pose.fuse()
    onnx_path = "../../yolo_model/yolo_v7_l_pose.onnx"
...
def onnx_video():
    ...
    while True:
        ...
        cv2.imshow("yolo_v7_pose_video", frame)
        cv2.waitKey(1)
