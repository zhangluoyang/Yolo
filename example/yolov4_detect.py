...
def torch_detect():
    param = Yolo4Param()
    model_path = "../../yolo_model/yolo_v4.pth"
    ...
    yolo_v4 = YoloV4(param=param)
    yolo_v4.load_state_dict(torch.load(model_path, map_location="cpu"))
    yolo_v4.eval()
    yolo_v4.cuda()

    predicts = yolo_v4.predict(tensor_dict={"batch_images": torch.tensor([data_dict["image"]]).cuda()})["predicts"]
...
def to_onnx():
    ...
    yolo_v4.cuda()
    yolo_v4.fuse()
    onnx_path = "../../yolo_model/yolo_v4.onnx"
    yolo_v4.to_onnx(onnx_path=onnx_path)
