...
from luoyang.yolov5_face.deprecated.YoloV5FaceLoss import YoloV5FaceLoss
from luoyang.yolov5_face.YoloV5FaceHead import YoloV5FaceHead
from luoyang.yolov5_face.Yolov5FaceBody import YoloV5FaceBody, Conv
...
class YoloV5Face(TaskLayer):

    def __init__(self, param: Yolo5FaceParam,
                 anchor_num: int = 3):
        super(YoloV5Face, self).__init__()
        self.param = param

        self.body_output_size = 5 + 2 * self.param.point_num + self.param.class_num
        self.body_net = YoloV5FaceBody(anchor_num=anchor_num,
    ...
    def build_loss_layer(self) -> List[LossLayer]:
        head_3_loss = YoloV5FaceLoss(loss_name="3",
                                     point_num=self.param.point_num,
                                     head_predict_name="head3_predicts",
                                     ground_true_name="head_3_ground_true",
                                     point_mask_name="head_3_point_mask",
                                     weight=4.0)
        head_4_loss = YoloV5FaceLoss(loss_name="4",
                                     point_num=self.param.point_num,
                                     head_predict_name="head4_predicts",
                                     ground_true_name="head_4_ground_true",
                                     point_mask_name="head_4_point_mask",
                                     weight=1.0)
        head_5_loss = YoloV5FaceLoss(loss_name="5",
                                     point_num=self.param.point_num,
                                     head_predict_name="head5_predicts",
                                     ground_true_name="head_5_ground_true",
                                     point_mask_name="head_5_point_mask",
                                     weight=0.4)
