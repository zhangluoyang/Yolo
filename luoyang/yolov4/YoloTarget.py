...
class YoloTarget(nn.Module):
    ...
    def __init__(self,
        ...
        :param stride: 缩放步长 (相对于输入层)
        :param body_output_size: 输出尺寸
        :param anchor_mask: 表明是哪一层的anchor
    ...
    def forward(self, batch_targets: List[Union[torch.Tensor, None]]) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        for b in range(batch_size):
            ...
            gt_box = torch.cat(
                (torch.zeros((target_num, 2), dtype=torch.float32, device=target.device), target[:, 2:4]),
                dim=-1)
            scale_anchors = torch.tensor(self.anchors, requires_grad=False, dtype=torch.float32,
                                         device=target.device) / self.stride
            anchors = torch.cat(
                (torch.zeros((self.anchor_num, 2), dtype=torch.float32, device=target.device), scale_anchors), dim=-1)
            gt_anchor_iou = torchvision.ops.box_iou(boxes1=gt_box, boxes2=anchors)

            best_anchor_index = torch.argmax(gt_anchor_iou, dim=-1).detach().cpu().numpy()
            for t, best_index in enumerate(best_anchor_index):
                # 保证 选择的 anchor 属于 该特征层
                ...
                c = target[t, 4].long()
                ground_true[b, :, j, i, 0] = target[t, 0]
                ground_true[b, :, j, i, 1] = target[t, 1]
                ground_true[b, :, j, i, 2] = target[t, 2]
                ground_true[b, :, j, i, 3] = target[t, 3]
                ground_true[b, :, j, i, 4] = 1
                ground_true[b, :, j, i, c + 5] = 1
