...
class YoloV5PoseHead(nn.Module):
    ...
    def __init__(self, stride: int,
        ...
        super(YoloV5PoseHead, self).__init__()
    ...
    def forward(self, input_tensor: torch.Tensor) -> torch.Tensor:
        ...
        # box and obj confidence
        input_tensor[..., :5] = torch.sigmoid(input_tensor[..., :5])
        # point confidence
        input_tensor[..., 5 + 2 * self.point_num: 5 + 3 * self.point_num] = \
            torch.sigmoid(input_tensor[..., 5 + 2 * self.point_num: 5 + 3 * self.point_num])
        # class
        input_tensor[..., 5 + 3 * self.point_num:] = \
            torch.sigmoid(input_tensor[..., 5 + 3 * self.point_num:])
        x = input_tensor[..., 0]
        y = input_tensor[..., 1]
        w = input_tensor[..., 2]
        h = input_tensor[..., 3]
        ...
        # 关键点的预测
        input_tensor[..., 5: 5 + 2 * self.point_num] = \
            (2 * input_tensor[..., 5: 5 + 2 * self.point_num] - 0.5) + _grid_xy
