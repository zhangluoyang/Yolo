...
class YoloCandidateTarget(nn.Module):
    ...
    def forward(self, predictions: List[torch.Tensor],
                targets: torch.Tensor) -> Tuple[List[Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]],
    List[torch.Tensor]]:
        """
         :param prediction [b, anchor, height, width, 4 + 1 + 2 * point_num + class_num ]
        ...
        for i in range(len(predictions)):
            ...
            # key point
            gain[6:6 + 2 * self.point_num] = torch.tensor(shape)[[3, 2] * self.point_num]
            # 最终目标在坐在特征层中坐标
...
class YoloSimpleOTATarget(nn.Module):
    ...
    def forward(self, predictions: List[torch.Tensor],
        ...
        :param targets: [[image_id, c, x, y, w, h, point1_x, point1_y,...]]
        :param indices: 候选的索引
        ...
        for batch_idx in range(batch_size):
            ...
            for layer_id, prediction in enumerate(predictions):
                ...
                # 预测的类别
                _p_cls.append(_candidate_prediction[:, -self.num_classes:])
                # 图像坐标系的坐标
