...
from luoyang.yolov7_face.YoloV7FaceLoss import YoloV7FaceLoss
from luoyang.yolov7_face.YoloV7FaceHead import YoloV7FaceHead
from luoyang.model.Layer import TaskLayer, LossLayer
from luoyang.yolov7_face.YoloV7FaceBody import YoloV7FaceBody, Conv, RepConv, _fuse_conv_and_bn
from luoyang.model.metric.DetectMetric import DetectMetric
from luoyang.yolov7_face.YoloTarget import YoloCandidateTarget, YoloSimpleOTATarget


class YoloV7Face(TaskLayer):

    def __init__(self, param: Yolo7FaceParam,
                 anchor_num: int = 3):
        super(YoloV7Face, self).__init__()
        self.param = param

        self.body_output_size = 5 + 2 * self.param.point_num + self.param.class_num

        self.body_net = YoloV7FaceBody(anchor_num=anchor_num,
                                       point_num=param.point_num,
                                       num_classes=param.class_num,
                                       phi=param.m_type,
                                       pretrained_path=self.param.pretrain_path)

        self.yolo_head3 = YoloV7FaceHead(stride=self.param.strides[0],
                                         point_num=param.point_num,
                                         input_size=(self.param.img_size // 8, self.param.img_size // 8),
                                         yolo_output_size=self.body_output_size,
                                         anchors=param.anchors[: 3])

        self.yolo_head4 = YoloV7FaceHead(stride=self.param.strides[1],
                                         point_num=param.point_num,
                                         yolo_output_size=self.body_output_size,
                                         input_size=(self.param.img_size // 16, self.param.img_size // 16),
                                         anchors=param.anchors[3: 6])

        self.yolo_head5 = YoloV7FaceHead(stride=self.param.strides[2],
                                         point_num=param.point_num,
    ...
    def build_loss_layer(self) -> List[LossLayer]:
        head_3_loss = YoloV7FaceLoss(loss_name="3",
                                     point_num=self.param.point_num,
                                     ...
                                     weight=4.0)
        head_4_loss = YoloV7FaceLoss(loss_name="4",
                                     point_num=self.param.point_num,
                                     ...
                                     weight=1.0)
        head_5_loss = YoloV7FaceLoss(loss_name="5",
                                     point_num=self.param.point_num,
