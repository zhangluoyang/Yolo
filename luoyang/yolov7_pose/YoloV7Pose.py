...
from luoyang.param.Param import Yolo7PoseParam
from luoyang.model.Layer import MetricLayer
import luoyang.utils.torch_utils as torch_utils
from luoyang.yolov7_pose.YoloV7PoseLoss import YoloV7PoseLoss
from luoyang.yolov7_pose.YoloV7PoseHead import YoloV7PoseHead
from luoyang.model.Layer import TaskLayer, LossLayer
from luoyang.yolov7_pose.YoloV7PoseBody import YoloV7PoseBody, Conv, RepConv, _fuse_conv_and_bn
from luoyang.model.metric.DetectMetric import DetectMetric
from luoyang.yolov7_pose.YoloTarget import YoloCandidateTarget, YoloSimpleOTATarget


class YoloV7Pose(TaskLayer):

    def __init__(self, param: Yolo7PoseParam,
                 anchor_num: int = 3):
        super(YoloV7Pose, self).__init__()
        self.param = param

        self.body_output_size = 5 + 3 * self.param.point_num + self.param.class_num
        self.body_net = YoloV7PoseBody(anchor_num=anchor_num,
                                       point_num=param.point_num,
                                       num_classes=param.class_num,
                                       phi=param.m_type,
                                       pretrained_path=self.param.pretrain_path)

        self.yolo_head3 = YoloV7PoseHead(stride=self.param.strides[0],
                                         point_num=param.point_num,
                                         input_size=(self.param.img_size // 8, self.param.img_size // 8),
                                         yolo_output_size=self.body_output_size,
                                         anchors=param.anchors[: 3])

        self.yolo_head4 = YoloV7PoseHead(stride=self.param.strides[1],
                                         point_num=param.point_num,
                                         yolo_output_size=self.body_output_size,
                                         input_size=(self.param.img_size // 16, self.param.img_size // 16),
                                         anchors=param.anchors[3: 6])

        self.yolo_head5 = YoloV7PoseHead(stride=self.param.strides[2],
                                         point_num=param.point_num,
                                         input_size=(self.param.img_size // 32, self.param.img_size // 32),
                                         yolo_output_size=self.body_output_size,
                                         anchors=param.anchors[6:])
        # 生成候选框
        self.candidate_target = YoloCandidateTarget(point_num=param.point_num,
                                                    anchors=[self.param.anchors[:3],
    ...
    def forward(self, tensor_dict: Dict[str, Union[torch.Tensor, List[torch.Tensor]]]):
        ...
        targets_v7 = tensor_dict["targets_v5"]
        # 与 yolo5 一样 生成候选正样本
    ...
    def build_loss_layer(self) -> List[LossLayer]:
        head_3_loss = YoloV7PoseLoss(loss_name="3",
                                     point_num=self.param.point_num,
                                     ...
                                     weight=4.0)
        head_4_loss = YoloV7PoseLoss(loss_name="4",
                                     point_num=self.param.point_num,
                                     ...
                                     weight=1.0)
        head_5_loss = YoloV7PoseLoss(loss_name="5",
                                     point_num=self.param.point_num,
