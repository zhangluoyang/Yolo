...
from luoyang.yolov6_face.YoloV6FaceBody import YoloV6FaceBody, RepVGGBlock, ConvModule
from luoyang.model.Layer import TaskLayer, LossLayer
from luoyang.model.metric.DetectMetric import DetectMetric
from luoyang.yolov6_face.YoloV6FaceLoss import YoloV6FaceLoss
import luoyang.utils.torch_utils as torch_utils


class YoloV6Face(TaskLayer):

    def __init__(self, param: Yolo6FaceParam):
        super(YoloV6Face, self).__init__()
        self.param = param
        self.yolo_body = YoloV6FaceBody(param=param)
    ...
    def forward(self, tensor_dict: Dict[str, Union[torch.Tensor, List[torch.Tensor]]]):
        ...
        (anchors_xx_yy, anchor_points_xy, num_anchors_list, stride_tensor, cls_score_tensor, predict_xy_xy,
         point_predict_xy, reg_distribute_tensor) = self.yolo_body.head(features)
    ...
    def _forward(self, images: torch.Tensor) -> List[torch.Tensor]:
        ...
        _, _, _, _, cls_score_tensor, predict_xy_xy, point_predict_xy, _ = self.yolo_body.head(features)
        # 保持与其它模型一致的输入
        ...
        batch_predicts = torch.cat([predict_cx_cy_w_h, predict_config, point_predict_xy, cls_score_tensor],
                                   dim=-1)
    ...
    def predict(self, tensor_dict: Dict[str, Union[torch.Tensor, List[torch.Tensor]]]) -> Dict[str, Any]:
        ...
        # [batch, anchor_num, 4 + 1 + 2 * point_num + class_num]
        batch_predicts = torch.cat([predict_xy_xy, predict_config, point_predict_xy, cls_score_tensor], dim=-1)

        batch_nms_predicts = torch_utils.non_max_suppression_with_point(prediction=batch_predicts,
    ...
    def build_loss_layer(self) -> List[LossLayer]:
        loss = YoloV6FaceLoss(target_name="targets_v6",
