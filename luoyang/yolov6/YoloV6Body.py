...
def build_network(m_type: str,
    ...
    return back_bone_network, neck_network, head_network, reg_max, use_dfl
...
class YoloV6Head(nn.Module):
    ...
    def bbox_decode(self,
        ...
        if self.use_dfl:
            ...
            # 加权求平均 计算最终值
            pred_dist = F.softmax(reg_distribute_tensor.view(batch_size, n_anchors, 4, self.reg_max + 1),
    ...
    def forward(self, x: List[torch.Tensor]):
        ...
        for i in range(self.num_layers):
            ...
            cls_output = self.cls_predict_list[i](cls_feat)  # [b, class_num, h, w]
            ...
            cls_score_list.append(cls_output.view(cls_output.size(0), self.num_classes, h * w)
                                  .permute(0, 2, 1))  # [batch, w*h, class_num]
            #  [batch, 4 * (reg_max + 1), h, w] ->  [batch, 4 * (reg_max + 1), h*w] -> [batch, h*w, 4 * (reg_max + 1)]
            if self.use_dfl:
                reg_distribute_list.append(
                    reg_output.view(reg_output.size(0), 4 * (self.reg_max + 1), h * w).permute(0, 2, 1))
            else:
                reg_distribute_list.append(
                    reg_output.view(reg_output.size(0), 4, h * w).permute(0, 2, 1))
...
class YoloV6Body(nn.Module):

    def __init__(self, param: Yolo6Param):
        super(YoloV6Body, self).__init__()
        self.param = param
        back_bone_network, neck_network, head_network, reg_max, use_dfl = build_network(m_type=param.m_type,
                                                                                        num_classes=len(
                                                                                            param.class_names),
        ...
        self.head = YoloV6Head(num_classes=param.class_num,
                               num_layers=param.num_layers,
