...
from luoyang.yolov6_pose.YoloV6PoseBody import YoloV6PoseBody, RepVGGBlock, ConvModule
from luoyang.model.Layer import TaskLayer, LossLayer
from luoyang.model.metric.DetectMetric import DetectMetric
from luoyang.yolov6_pose.YoloV6PoseLoss import YoloV6PoseLoss
import luoyang.utils.torch_utils as torch_utils


class YoloV6Pose(TaskLayer):

    def __init__(self, param: Yolo6PoseParam):
        super(YoloV6Pose, self).__init__()
        self.param = param
        self.yolo_body = YoloV6PoseBody(param=param)
    ...
    def forward(self, tensor_dict: Dict[str, Union[torch.Tensor, List[torch.Tensor]]]):
        ...
        anchors_xx_yy, anchor_points_xy, num_anchors_list, stride_tensor, cls_score_tensor, point_conf_tensor, \
        predict_xy_xy, point_predict_xy, reg_distribute_tensor = self.yolo_body.head(features)
        ...
        tensor_dict["point_conf_tensor"] = point_conf_tensor
        tensor_dict["predict_xy_xy"] = predict_xy_xy
        tensor_dict["point_predict_xy"] = point_predict_xy
    ...
    def _forward(self, images: torch.Tensor) -> List[torch.Tensor]:
        ...
        _, _, _, _, cls_score_tensor, point_conf_tensor, \
        predict_xy_xy, point_predict_xy, _ = self.yolo_body.head(features)
        # 保持与其它模型一致的输入
        ...
        # [b, h, w, 3 * point_num]
        point_xyc = torch.cat((point_predict_xy, point_conf_tensor), dim=-1)

        batch_predicts = torch.cat([predict_cx_cy_w_h, predict_config, point_xyc, cls_score_tensor], dim=-1)
    ...
    def predict(self, tensor_dict: Dict[str, Union[torch.Tensor, List[torch.Tensor]]]) -> Dict[str, Any]:
        ...
        predict_config = torch.clamp(cls_score_tensor[:, :, 0:1], min=1, max=1)
        point_conf_tensor = tensor_dict["point_conf_tensor"]
        point_predict_xy = tensor_dict["point_predict_xy"]
        # [b, h, w, 3 * point_num]
        point_xyc = torch.cat((point_predict_xy, point_conf_tensor), dim=-1)

        # [batch, anchor_num, 4 + 1 + 2 * point_num + point_num + class_num]
        batch_predicts = torch.cat([predict_xy_xy, predict_config, point_xyc, cls_score_tensor], dim=-1)
    ...
    def build_loss_layer(self) -> List[LossLayer]:
        loss = YoloV6PoseLoss(target_name="targets_v6",
                              ...
                              point_predict_xy_name="point_predict_xy",
                              predict_score_name="cls_score_tensor",
                              point_conf_name="point_conf_tensor",
