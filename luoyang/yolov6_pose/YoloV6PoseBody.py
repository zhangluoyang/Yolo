...
def create_head(channels_list: List[int],
    ...
    head_layers = nn.Sequential(ConvBNSiLU(in_channels=channels_list[chx[0]],
                                ...
                                nn.Conv2d(in_channels=channels_list[chx[0]],
                                          out_channels=(num_classes + point_num) * num_anchors,
                                          kernel_size=(1, 1)),
                                nn.Conv2d(in_channels=channels_list[chx[0]],
...
class YoloV6PoseHead(nn.Module):
    ...
    def __init__(self,
                 ...
                 use_dfl: bool = False,
                 reg_max: int = 16):
        super(YoloV6PoseHead, self).__init__()
    ...
    def forward(self, x: List[torch.Tensor]):
        ...
        for i in range(self.num_layers):
            ...
            # 类别预测 [batch, class_num + point_num, w, h]
            cls_output = torch.sigmoid(cls_output)
            # [batch, key_point_num, h, w]
            point_conf_output = cls_output[:, self.num_classes:, :, :]
            # [batch, h*w, key_point_num]
            point_conf_list.append(
                point_conf_output.view(point_conf_output.size(0), self.point_num, h * w).permute(0, 2, 1))
        ...
        point_conf_tensor = torch.cat(point_conf_list, dim=1)
        ...
        return anchors_xx_yy, anchor_points_xy, num_anchors_list, \
               stride_tensor, cls_score_tensor, point_conf_tensor, predict_xy_xy, point_predict_xy, reg_distribute_tensor


class YoloV6PoseBody(nn.Module):

    def __init__(self, param: Yolo6PoseParam):
        super(YoloV6PoseBody, self).__init__()
        self.param = param
        ...
        self.head = YoloV6PoseHead(num_classes=param.class_num,
                                   num_layers=param.num_layers,
