...
class ATAssigner(nn.Module):
    ...
    def forward(self, anchors_xx_yy: torch.Tensor,
                ...
                ground_true_xx_yy: torch.Tensor,
                ground_true_point: torch.Tensor,
        ...
        target_gt_idx, fg_mask, mask_pos = select_highest_overlaps(mask_pos,
                                                                   overlaps,
                                                                   self.n_max_boxes)
        # [batch, anchor_num] [batch, anchor_num, 4] [batch, anchor_num, 2 * point_num] [batch, anchor_num, class_num]
        target_labels, target_xy_xy, target_point_xy, target_scores = self.get_targets(
        ...
        return target_labels.long(), target_xy_xy, target_point_xy, target_scores, fg_mask.bool()
    ...
    def get_targets(self,
        ...
        target_point_xy = ground_true_point.reshape([-1, 2 * self.point_num])[target_gt_idx.flatten()]
        target_point_xy = target_point_xy.reshape([self.bs, self.n_anchors, 2 * self.point_num])
        ...
        return target_labels, target_xy_xy, target_point_xy, target_scores
...
class TaskAlignedAssigner(nn.Module):
    ...
    def forward(self, predict_xy_xy: torch.Tensor,
        ...
        :param ground_true_point: [batch_size, n_max_boxes, 2 * point_num]
        ...
        for i in range(cycle):
            ...
            gt_point = ground_true_point[start:end, ...]
            ...
            # assigned target
            target_labels, target_xy_xy, target_point_xy, target_scores = self.get_targets(
                ground_true_labels=gt_labels_,
                ground_true_xx_yy=gt_xy_xy_,
                ground_true_point=gt_point,
                target_gt_idx=target_gt_idx,
                fg_mask=fg_mask)
            ...
            target_point_xy_list.append(target_point_xy)
        ...
        target_point_xy = torch.cat(target_point_xy_list, 0)
        target_scores = torch.cat(target_scores_lst, 0)
        fg_mask = torch.cat(fg_mask_lst, 0)

        return target_labels, target_xy_xy, target_point_xy, target_scores, fg_mask.bool()
    ...
    def get_targets(self,
        ...
        # assigned point
        target_point_xy = ground_true_point.reshape([-1, 2 * self.point_num])[target_gt_idx]
        ...
        return target_labels, target_xx_yy, target_point_xy, target_scores
