...
from luoyang.yolov6_pose.YoloV6PoseTarget import *
from luoyang.model.loss.BceSmoothLoss import BceSmoothLoss


class YoloV6PoseLoss(LossLayer):
    ...
    def __init__(self,
                 ...
                 reg_max: int = 16,
                 warmup_epoch: int = 4):
        super(YoloV6PoseLoss, self).__init__()
        ...
        self.point_conf_name = point_conf_name
    ...
    def loss_feed_forward(self, tensor_dict: Dict[str, Union[torch.Tensor, List[torch.Tensor]]]) -> \
        ...
        # [batch, h*w, point_num]
        point_conf_tensor = tensor_dict[self.point_conf_name]
            ...
            if self.current_epoch < self.warmup_epoch:
                target_labels, ground_true_xx_yy, ground_true_point_xy, target_scores, fg_mask = \
                    self.warmup_assigner(anchors_xx_yy=anchors_xx_yy,
                                         ...
                                         predict_xy_xy=predict_xy_xy.detach())
            else:
                target_labels, ground_true_xx_yy, ground_true_point_xy, target_scores, fg_mask = \
            ...
            if self.current_epoch < self.warmup_epoch:
                ...
                target_labels, ground_true_xx_yy, ground_true_point_xy, target_scores, fg_mask = \
                    self.warmup_assigner(anchors_xx_yy=anchors_xx_yy.cpu(),
                                         ...
                                         ground_true_point=ground_true_point.cpu(),
        ...
        if num_pos > 0:
            ...
            # [num_pos, point_num]
            _point_predict_conf = point_conf_tensor[fg_mask]
            # [num_pos, point_num * 3]
            p_kpt = _point_predict_xy
            # [num_pos, point_num]
            kpt_mask = torch.logical_and(torch.gt(_ground_true_point_xy[:, 0::2], 0),
                                         torch.gt(_ground_true_point_xy[:, 1::2], 0))
            # [num_pos, point_num * 2]
            gt_kpt = _ground_true_point_xy

            scale_ground_true_xx_wh = torchvision.ops.box_convert(boxes=scale_ground_true_xx_yy[fg_mask],
                                                                  in_fmt="xyxy",
                                                                  out_fmt="cxcywh")

            area = scale_ground_true_xx_wh[:, 2:].prod(1, keepdim=True)

            # d = (p_kpt[..., 0::2] - gt_kpt[..., 0::2]) ** 2 + (p_kpt[..., 1::2] - gt_kpt[..., 1::2]) ** 2
            # d = d * kpt_mask
            # point_loss = torch.sum(d) / (0.001 + torch.sum(kpt_mask)) * self.point_weight
            point_loss = self.point_loss(p_kpt=p_kpt,
                                         gt_kpt=gt_kpt,
                                         kpt_mask=kpt_mask,
                                         area=area) * self.point_weight
            # [bs, anchor_num, point_num]
            point_conf_target = kpt_mask.to(point_conf_tensor.dtype)

            point_conf_loss = self.point_conf_loss(_point_predict_conf,
                                                   point_conf_target) * self.point_conf_weight



        else:
            point_loss, point_conf_loss = 0, 0
        loss = self.class_loss_weight * loss_cls + self.iou_loss_weight * loss_iou + self.dfl_loss_weight * loss_dfl
        loss += point_conf_loss
        loss += point_loss
        # print(point_loss)
