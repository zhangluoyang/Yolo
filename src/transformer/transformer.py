...
class Rotate(Transformer):
    ...
    def transformer(self, data_dict: dict):
        ...
        if random.random() > 0.5:
            ...
            rotate = cv2.getRotationMatrix2D((height * 0.5, width * 0.5), angle, 1)
            image = cv2.warpAffine(image, rotate, (height, width))
            rect_targets = copy.deepcopy(data_dict["rect_targets"])
            left_up_points = np.ones(shape=(len(rect_targets), 3), dtype=np.float32)
            left_up_points[:, :2] = rect_targets[:, (0, 1)]
            right_down_points = np.ones(shape=(len(rect_targets), 3), dtype=np.float32)
            right_down_points[:, :2] = rect_targets[:, (2, 3)]
            # 变换矩阵
            left_up_points = np.dot(left_up_points, rotate.T)
            right_down_points = np.dot(right_down_points, rotate.T)
            # 左上右下
            left_up_right_down_points = np.concatenate((left_up_points, right_down_points), axis=-1)
            data_dict["rect_targets"][:, 0] = np.min(left_up_right_down_points[:, (0, 2)], axis=1)
            data_dict["rect_targets"][:, 1] = np.min(left_up_right_down_points[:, (1, 3)], axis=1)
            data_dict["rect_targets"][:, 2] = np.max(left_up_right_down_points[:, (0, 2)], axis=1)
            data_dict["rect_targets"][:, 3] = np.max(left_up_right_down_points[:, (1, 3)], axis=1)
            data_dict["image"] = image
                ...
                for t, ratio in enumerate(max_ratios):
                    ...
                    for k, mask in enumerate(self.anchors_mask[_layer_id]):
                        ...
                        for offset in offsets:
                            ...
                            y_true[_layer_id][k, local_j, local_i, 0] = batch_target[t, 0]
                            y_true[_layer_id][k, local_j, local_i, 1] = batch_target[t, 1]
                            y_true[_layer_id][k, local_j, local_i, 2] = batch_target[t, 2]
                            y_true[_layer_id][k, local_j, local_i, 3] = batch_target[t, 3]
