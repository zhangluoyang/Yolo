...
from src.model.Layer import TaskLayer
...
class DistributedDataParallel(nn.Module):
    ...
    def forward(self, *inputs, **kwargs):
        with torch.autograd.profiler.record_function("DistributedDataParallel.forward"):
            ...
            if self.device_ids:
                output = self.module.feed_forward(tensor_dict=inputs[0])
            else:
                output = self.module.feed_forward(tensor_dict=inputs[0])
    ...
    def update_fine_tune_param(self):
        self.module.update_fine_tune_param()

    def update_train_param(self):
        self.module.update_train_param()

    def predict(self, tensor_dict: Dict[str, Union[torch.Tensor, List[torch.Tensor]]]) -> Dict[str, Any]:
        return self.module.predict(tensor_dict=tensor_dict)

    def build_loss_layer(self):
        return self.module.build_loss_layer()

    def build_metric_layer(self):
        return self.module.build_metric_layer()
